...
def action_list_accounts(ctx):
    ...
    s, data = whm_api(*ctx[:6], "listaccts", {"search": "", "searchtype": "user"}, ctx[-1])
    if isinstance(data, dict):
...
def action_read_file(ctx, path: str):
    ...
    s, data = whm_api(*ctx[:6], "getlocalpackage", {"package": path}, ctx[-1])
...
def action_add_admin(ctx, username: str, password: str):
    ...
    if isinstance(data, dict):
        ...
        if isinstance(result, list) and result:
            ...
            reason = result[0].get("statusmsg", "")
            log("API", f"createacct → status={status} {reason}")
        else:
            log("API", f"createacct → {str(data)[:200]}")
    # Step 2: grant reseller privileges
    s2, data2 = whm_api(*ctx[:6], "setupreseller",
                        {"user": username, "makeowner": 1}, ctx[-1])
    log("API", f"setupreseller → HTTP {s2}")
    # Step 3: grant all ACLs
    s3, data3 = whm_api(*ctx[:6], "setacls",
                        {"user": username, "all": 1}, ctx[-1])
    log("API", f"setacls → HTTP {s3}")
    safe_print(f"\n{C.GREEN}[+] Admin account created: {username} / {password}{C.RESET}\n")

def action_version(ctx):
    """Get cPanel version"""
    s, data = whm_api(*ctx[:6], "version", {}, ctx[-1])
